...
The agent-facing surface must not change. Agents keep writing files and running `jardin sync`;
they never learn that git exists. See `~/.jardin/memory/conventions/jardin-agent-surface.md`.
...
One commit per operation, never per file: one `jardin sync` that pulls six changes is one
...
succeed. The failure goes to the human, never to the agent — the same shape as `tools.Jardin()`
in nacelle returning no tools when jardin is absent rather than erroring.
...
**`jardin sync` gains no arguments.** A commit message here serves one reader: a human scanning
...
log line:  lint | jardin-agent-surface: corrected — sync is domain, not a leak; agents keep
           `jardin sync` and reads stay explicit. Only .conflict files in memory/ and
...
every mechanical step, and most of jardin's will be `sync: pulled 4 from lucy`. `log.md` is
...
`~/.jardin/.conflicts/` instead. Already excluded from sync, so it is local clutter only.
...
Recommended freshness metadata is `written_at`, `last_confirmed_at`, `expires_at`. Jardin has the
...
The recommended write gate has four conditions and jardin's has three; "secrets never enter
memory" is missing. Flows already carry the rule.

### B6. An episodic-to-semantic consolidation stage

The largest item, and the one the field calls crucial. Jardin captures episodically (`events/`,
...
- **Not CRDTs.** They guarantee convergence without conflicts, and that is the wrong goal here.
  Jardin stores claims: a CRDT merges "X is true" and "X is false" into one page and calls it
...
### Does jardin hold normative documents, and if so under what rules?

Jardin's memory is **descriptive**: dated observations carrying provenance, written mostly by
...
jardin's current model distinguishes the two kinds of page, which means an agent editing a
...
jardin, so it wants deciding before Track A finishes rather than after.
...
**And the threat model here is milder than the papers'.** They assume an adversary. Jardin's
...
- `jardin memory search` returns findings and `log.md` lines, and never a commit message.
