# `jardin flow` — typed steps, the graph, and history
...
`~/.jardin/extensions/models/<type>.ts` (the leading `@` is dropped, so
...
jardin never reads the TypeScript — only the schema. That is the whole point of
...
```console
$ jardin flow trust-model @acme/http-check
```
...
`--since` reuses the same parser as `jardin stats`. A window parser that
...
machine running an older jardin refuses a flow that uses them rather than
running it wrong. **Update jardin on every machine that runs the flow before
adding them.**
