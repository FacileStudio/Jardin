...
It is dormant until `OLLAMA_URL` is set, and it is the only part of Jardin that needs
...
default and the right answer at this size: it keeps the index inside the `jardin-data`
...
`jardin-data` volume (`flat`) or in qdrant's own.
