# `mycelium flow` — v2: data chaining
...
| Binding `MYCELIUM_TOKEN` | `may not bind MYCELIUM_TOKEN` |
...
- **A producing step cannot invent a binding.** Bindings are declared by the
  *consumer*, in `needs`. This is why `mycelium flow` has no equivalent of the
...
`mycelium flow show` prints those lines under the step, sorted by name, so a run
...
running a mycelium older than this release will therefore refuse a flow that uses
...
local: **update mycelium on every machine that runs the flow before adding
`needs` to it.**
