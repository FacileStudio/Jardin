...
| Agent | Writes |
|---|---|
| `agents` | `~/.agents/AGENTS.md`, skills as `~/.agents/skills/<name>/SKILL.md` — the cross-vendor standard path, read by nacelle and any tool following the AGENTS.md spec. Its `AGENTS.md` opens with a generated-by notice; `jardin diff agents` first if the file may be handwritten |
