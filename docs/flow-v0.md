...
- `mycelium flow run|list|show|runs|trust`
- YAML flow files under `~/.mycelium/flows/`, synced like every other tree
- One JSON run artifact per execution under `~/.mycelium/runs/`, never synced
...
- porte. Mycelium's server auth is already scoped, hashed at rest, rate limited
...
`~/.mycelium/runs/<flow>/<rfc3339-nano>.json`, mode 0600, directory 0700:
...
| Command | Behaviour |
|---|---|
| `mycelium flow list` | Flow names, step counts, trust state |
...
code that runs on lucy and ruche. Today mycelium distributes prose an agent reads
...
Strip `MYCELIUM_TOKEN` from the child environment. A flow distributed by mycelium
...
`mycelium flow run deploy-check` on ruche produces an artifact, `mycelium flow runs
deploy-check --json` lists it, the flow file syncs to lucy, and the first run
there refuses until `mycelium flow trust deploy-check` is accepted.
