...
The CLI needs nothing else — `jardin init` scaffolds `~/.jardin` and every command works
offline until you `jardin login`.

To run the server against a throwaway data directory:

```sh
go run . serve --data /tmp/jardin-dev --port 8420
```
