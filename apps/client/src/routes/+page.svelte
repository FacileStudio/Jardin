<script lang="ts">
	...
	onMount(() => {
		if (localStorage.getItem('mycelium.token')) {
			goto('/memory');
			return;
		}
		visible = true;
...
<div class="min-h-screen bg-white text-zinc-900">
	...
	<main>
		...
		<section class="border-y border-zinc-200 bg-zinc-950 text-white">
			<div class="mx-auto max-w-5xl px-6 py-28 md:py-36">
				...
				<p use:reveal={{ delay: 200 }} class="mt-8 max-w-xl text-lg leading-relaxed text-zinc-400">
					Vos règles et compétences sont écrites en markdown. Mycelium génère automatiquement les fichiers de configuration pour chaque agent.
				</p>
		...
		<section class="bg-zinc-950 text-white">
			<div class="mx-auto max-w-5xl px-6 py-28 md:py-36">
				...
				<p use:reveal={{ delay: 200 }} class="mt-8 max-w-xl text-lg leading-relaxed text-zinc-400">
					<code class="rounded bg-zinc-800 px-2 py-0.5 text-sm font-mono">mycelium serve</code> lance un serveur de sync HTTP. Déployez-le sur votre VPS, connectez vos machines avec un token.
				</p>
				...
				<div use:reveal={{ delay: 300 }} class="mt-12 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 font-mono text-sm">
					...
					<p class="text-green-400">$ mycelium serve --port 8420</p>
					<p class="mt-4 text-zinc-500"># sur votre machine</p>
					<p class="text-green-400">$ mycelium sync</p>
	...
	<footer class="border-t border-zinc-200">
		<div class="mx-auto max-w-5xl px-6 py-6 text-center text-sm text-zinc-400">
			&copy; {new Date().getFullYear()} Mycelium by <a href="https://facile.studio" class="text-zinc-600 transition-colors hover:text-zinc-900">Facile.</a>
		</div>
	</footer>
</div>
{/if}
