...
<div class="flex flex-col gap-10">
	...
	>
		...
		<p class="text-fc-xs text-fc-fg-muted">
			It assumes the <code class="font-fc-mono">jardin</code> CLI is installed and logged in with
	...
	{#if createdToken}
		<div class="flex flex-col gap-4">
			<Alert tone="warning" title="Copy it now">
				This is the only time the token is shown. Jardin stores a hash, so it cannot be shown
			...
			<SecretField
				...
				autoHideMs={0}
				label="Token"
				helper="Run `jardin login` on the machine and paste it when asked."
			/>
