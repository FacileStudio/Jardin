<script lang="ts">
	...
	async function save() {
		saving = true;
		try {
			await backend.ruleSave(name, draft);
			content = draft;
			editing = false;
		} finally {
			saving = false;
...
<div class="space-y-6">
	<a href="/rules" class="inline-flex items-center gap-1 text-sm text-muted-foreground transition-colors hover:text-foreground">
	...
	<div class="flex items-start justify-between gap-4">
		<div class="flex items-center gap-3">
			<div class="flex size-10 items-center justify-center rounded-xl bg-accent">
				<Icon icon="solar:ruler-angular-linear" class="size-5 text-foreground" />
			</div>
			<div>
				<h2 class="text-xl font-semibold tracking-tight">{name}</h2>
				<p class="font-mono text-xs text-muted-foreground">rules/{name}.md</p>
			</div>
		</div>
