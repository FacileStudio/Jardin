...
func fetchSpaces(cfg *config.JardinConfig) ([]spaceInfo, error) {
	if cfg.ServerURL() == "" {
...
func resolveSpace(spaces []spaceInfo, arg string) (*spaceInfo, error) {
	...
	return nil, fmt.Errorf("no space matching %q — run 'jardin spaces list'", arg)
}

func setSpace(cfg *config.JardinConfig, spaceID string) error {
