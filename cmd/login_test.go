...
func TestTheEnvironmentOverridesTheConfigFile(t *testing.T) {
	...
	if err := config.SaveJardinConfig(stored); err != nil {
		t.Fatal(err)
	}
...
func TestLogoutClearsTheTokenAndKeepsEverythingElse(t *testing.T) {
	...
	after, err := config.LoadJardinConfig()
	if err != nil {
		t.Fatal(err)
	}
	if after.Token != "" {
	...
	info, err := os.Stat(filepath.Join(home, ".jardin.yml"))
	if err != nil {
		t.Fatal(err)
	}
