// Package env loads the Jardin server's configuration from the process
...
// DefaultPort is the port `jardin serve` binds when PORT is unset.
...
// troncenv.LoadCore: LoadCore requires DATABASE_URL and Jardin has no database,
