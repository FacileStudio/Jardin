...
// nothing else deletes it, so doctor would stay red forever. Syncing the path
// clears it.
func TestSyncSingleWriterClearsAStaleConflictBackup(t *testing.T) {
	c, clientDir, serverDir := setup(t)
	establishBase(t, c, clientDir, serverDir, "usage/ruche/current.json", `{"used":1}`)
	...
	if exists(clientDir, "usage/ruche/current.json.conflict") {
		t.Fatal("a stale backup on a single-writer path survived the sync")
	}
}
