...
func TestFoldMergesWithinGap(t *testing.T) {
	...
	sealed := fold(state, "lucy", []Event{
		ev(0, "Jardin", 100),
		ev(5*time.Minute, "Jardin", 50),
		ev(14*time.Minute, "Jardin", 25),
	}, t0.Add(20*time.Minute))

	if len(sealed) != 0 {
		t.Fatalf("expected no sealed blocks, got %d", len(sealed))
	}
	open := state.Open["jardin|claude"]
	if open == nil {
...
func TestFoldSealsStaleOpenBlocks(t *testing.T) {
	state := newScanState()
	fold(state, "lucy", []Event{ev(0, "Jardin", 100)}, t0.Add(1*time.Minute))
...
func TestRepoNameFromRemote(t *testing.T) {
	cases := map[string]string{
		...
		"https://github.com/FacileStudio/Jardin":    "Jardin",
...
func TestBlockIDDeterministic(t *testing.T) {
	a := Block{Project: "Jardin", Machine: "lucy", Agent: "claude", StartedAt: t0}
	b := Block{Project: "Jardin", Machine: "lucy", Agent: "claude", StartedAt: t0}
	if a.computeID() != b.computeID() {
		t.Fatal("same natural key must yield same id")
	}
	c := Block{Project: "Jardin", Machine: "ruche", Agent: "claude", StartedAt: t0}
