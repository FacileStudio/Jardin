...
func TestClaimSaveLoadRelease(t *testing.T) {
	...
	if err := ReleaseClaim(dir, "Jardin", "lucy", "pi"); err != nil {
		t.Fatal(err)
	}
	if got, _ := LoadClaim(dir, "Jardin", "lucy", "pi"); got != nil {
...
func TestClaimPerAgentPathsDoNotCollide(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		SaveClaim(dir, &Claim{
			Project: "Jardin", Machine: tc.machine, Agent: tc.agent,
...
func TestReadClaimsLiveResolvesLiveness(t *testing.T) {
	...
	SaveClaim(dir, &Claim{
		Project: "Jardin", Machine: "lucy", Agent: "pi", Task: "fresh",
		StartedAt: now.Add(-5 * time.Minute), UpdatedAt: now.Add(-10 * time.Second),
	})
	SaveClaim(dir, &Claim{
		Project: "Jardin", Machine: "lucy", Agent: "claude", Task: "idle",
	...
	entries := ReadClaimsLive(dir, "Jardin", now)
	if len(entries) != 2 {
		t.Fatalf("expected 2 claims for Jardin, got %d", len(entries))
	}
