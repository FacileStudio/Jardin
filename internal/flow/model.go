...
const (
	...
	modelExt = ".ts"
	// modelRuntime executes a model. It is not a dependency of mycelium: a flow
...
func LoadModel(typeName string) (*Model, error) {
	...
	if pinned == "" {
		return nil, fmt.Errorf("model %q is not trusted on this machine; read %s then run: mycelium flow trust-model %s",
...
// and reads JSON back, so adding a model needs no mycelium release.
