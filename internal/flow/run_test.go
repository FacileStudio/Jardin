...
func TestExecuteStripsMyceliumToken(t *testing.T) {
	t.Setenv("MYCELIUM_TOKEN", "leakme")
	run := execFlow(t, []Step{{Name: "env", Run: `echo "[$MYCELIUM_TOKEN]"`}}, Options{})

	if strings.Contains(run.Steps[0].Stdout, "leakme") {
		t.Fatalf("MYCELIUM_TOKEN reached the child: %q", run.Steps[0].Stdout)
	}
