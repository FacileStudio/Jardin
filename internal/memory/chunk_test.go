...
const samplePage = `---
title: jardin flow — recorded procedures
type: tool
confidence: high
---
...
func TestChunksSplitOnFindingBlocks(t *testing.T) {
	chunks := Chunks("tools/jardin-flow.md", samplePage)
	if len(chunks) != 3 {
...
func TestChunkTextCarriesThePageIdentity(t *testing.T) {
	chunks := Chunks("tools/jardin-flow.md", samplePage)
	text := chunks[1].Text()
	for _, want := range []string{"tools/jardin-flow", "jardin flow — recorded procedures", "tool", "The trust store is lazy"} {
