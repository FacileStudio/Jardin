...
func TestSearchRemoteHappyPath(t *testing.T) {
	...
	url := remoteServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeFakeJSON(w, map[string]any{"results": []map[string]any{
			{"path": "tools/mycelium.md", "line": 12, "score": 0.0164, "excerpt": "a finding"},
		}})
	})
	...
	if len(results) != 1 || results[0].Path != "tools/mycelium.md" || results[0].Line != 12 {
		t.Fatalf("results = %+v, want one hit for tools/mycelium.md:12", results)
	}
