...
// TestTheCLIFlowEndToEnd walks the path a `jardin login` takes: start with the
...
// The same walk without a nonce, which is what a jardin binary installed before
...
func TestABrowserLoginStillEndsOnTheWebApp(t *testing.T) {
	...
	srv.OIDC = &env.OIDC{
		Issuer: idp.URL, ClientID: clientID, ClientSecret: "secret",
		RedirectURL: ts.URL + "/api/auth/oidc/callback", SuccessURL: "https://jardin.example.com/auth/callback",
	}
	...
	if !strings.HasPrefix(location, "https://jardin.example.com/auth/callback#token=") {
