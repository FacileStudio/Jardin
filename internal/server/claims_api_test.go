...
func TestClaimsListReturnsJSON(t *testing.T) {
	...
	if !strings.Contains(rec.Body.String(), `"project":"Mycelium"`) {
...
func TestClaimsReleaseRemovesClaim(t *testing.T) {
	...
	req := httptest.NewRequest("DELETE", "/api/claims/Mycelium/lucy/pi", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	claim, err := sessions.LoadClaim(s.DataDir, "Mycelium", "lucy", "pi")
	if err != nil {
		t.Fatal(err)
	}
	if claim != nil {
