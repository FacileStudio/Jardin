...
- **Config**: YAML (`~/.jardin.yml`) for the CLI, environment for the server (`internal/env`)
...
- One binary, one container, one Traefik router `Host(jardin.facile.studio)` — no
