...
Give `~/.jardin` a history so a page can be diffed, blamed and reverted, and make memory age
...
raise a threshold. `conventions/jardin-agent-surface.md` — the agent-facing surface must not gain
a word. `conventions/multi-agent-git-hygiene.md` — commit explicit paths, this checkout is shared.

**Does not apply, stated so a worker does not look for it**: migrations (jardin has no database),
auth/porte (nothing here touches identity), muse (no UI), events (nothing emitted),
module path (jardin is flat `github.com/FacileStudio/Jardin`, not the `apps/api` shape),
...
`~/.jardin/rules/` (the shared rules, not this repo), and this repo's `AGENTS.md` if it restates
the gate

The recommended write gate has four conditions and jardin's has three. Add: **secrets never enter
...
**Exit**: the gate text names secrets; `jardin install --all` regenerates every agent's config
with it.
...
- `Commit(dataDir, message string, paths []string)` — stages **explicit paths**, never add-all.
  Author from `config.LoadJardinConfig().Machine`.
...
**Exit**: `~/.jardin/.git` exists after `jardin init`; `git log` in that directory shows commits;
`jardin sync` produces exactly one commit per run; `events/` never appears in `git status`.

### 5. Commit on the write and sync paths  `[filet]`

`cmd/sync.go`, `internal/sync/sync.go`

One commit per operation, never per file. A `jardin sync` that pulls six changes is one commit.

Messages are **mechanical and derived** — `sync: pulled 4 from lucy, pushed 2`. They serve one
reader: a human scanning `jardin memory log`. **`jardin sync` gains no arguments.** An earlier
...
as nacelle's `tools.Jardin()` returning no tools when jardin is absent rather than erroring.
...
`jardin memory log [path]`, `jardin memory diff <ref> [path]`, `jardin memory revert <ref>`.
...
**Exit**: a page can be deleted, then restored with `jardin memory revert`, and `log` names the
...
**Exit**: `jardin memory search` returns findings and `log.md` lines, and never a commit message.
...
into an agent's view. Write under `~/.jardin/.conflicts/<path>` instead. Already excluded from
...
The largest item and the last. Jardin captures episodically (`events/`, sessions) and holds
...
| `~/.jardin/rules/` | secrets clause (2) — outside this repo |

## Exit criteria

- A page can be deleted and restored with `jardin memory revert`, and the history names the
  machine that changed it.
- A mass deletion is refused without an explicit flag.
- `jardin memory search` returns findings and `log.md` lines, never a commit message.
...
- **The eval scores the live wiki.** `internal/memory/eval_test.go` reads `~/.jardin/memory` and
